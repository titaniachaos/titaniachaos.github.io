...
/**
 * One page per word in the vocabulary, in German.
 *
  ...
  paths() {
    ...
    return TAGS.map((category) => ({
      params: {
        ...
        name: asTitle(TAG_NAMES['de'][category]),
        title: fill(ui.title, asTitle(TAG_NAMES['de'][category])),
        description: fill(ui.description, TAG_NAMES['de'][category])
      }
    }))
  }
}
